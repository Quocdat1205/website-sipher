...
export const SignInModal = () => {
    ...
    useQuery("location", () => axios.get("https://ip.nf/me.json"), {
        ...
        onSuccess: (data: any) => {
            setValueSelect(
                dataCountry.find(
                    (item: any) => item.code === data.data.ip.country_code
                )
                    ? {
                          code: data.data.ip.country_code,
                          name: data.data.ip.country,
                      }
                    : {
                          name: "Afghanistan",
                          code: "AF",
    ...
    const handleSign = async () => {
        setIsLoading(true)
        try {
            const tracking = await trackIP(wallet.account, valueSelect.name)
            if (tracking) {
                setIsLoading(false)
                onClose()
            } else {
                ...
                toast({
                    ...
                    title: "Error",
                    message:
                        "Thank you for your interest in taking part in our Airdrops. However, due to restrictions and our ongoing compliance efforts, we will be unable to provide this offering to you.",
                })
            }
        } catch (error: any) {
            setIsLoading(false)
                    ...
                    <Box p={4} w="full">
                        ...
                        <CheckboxGroup colorScheme="orange">
                            ...
                            >
                                ...
                                >
                                    <Text>
                                        I declare that I am NOT a resident of
                                        the prohibited territories or
