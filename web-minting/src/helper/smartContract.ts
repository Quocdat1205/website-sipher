...
export const sendSmartContract = async (input: SendSCInput) => {
	const { address, slot, slotPrice, proof, privateCap, freeCap } = input
	...
	await ContractProviderSALE.methods.buy(slot, proof, privateCap, freeCap).send({
		from: address,
 ...
 */
export const getSaleConfig = async (): Promise<[number, number, number, number]> => {
	const data = await ContractProviderSALE.methods.getSaleConfig().call()
	return [parseInt(data[0]), parseInt(data[1]), parseInt(data[2]), parseInt(data[3])]
}
