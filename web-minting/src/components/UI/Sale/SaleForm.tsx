...
const SaleForm = ({ mode }: SaleFormProps) => {
	...
	const mint = async () => {
		...
		} else if (userRecord && userRecord.whitelistBought >= metaState.isWhitelisted.privateCap) {
			toast({
				status: "error",
				title: "Confirmation error!",
				message: `You can only buy up to ${metaState.isWhitelisted.privateCap} NFTs`,
			})
			return
		}
		...
		const {
			...
			isWhitelisted: { freeCap, privateCap, proof },
		} = metaState
		await sendSmartContract({ address, slot, slotPrice, proof, privateCap, freeCap })
	...
	const getMaxSlot = () => {
		if (mode === "public") {
			...
			return userRecord ? Math.max(metaState.isWhitelisted.privateCap - userRecord.whitelistBought, 0) : 0
		}
	}
		...
		<Flex flex={1} flexDir="column" ml={4}>
			...
			<Flex justify="space-between" mt="2">
				...
				<MyText>{`${userRecord && userRecord[mode === "public" ? "publicBought" : "whitelistBought"]} / ${
					mode === "public" ? PUBLIC_CAP : metaState.isWhitelisted.privateCap
				}`}</MyText>
			</Flex>
			<Counter
				value={slot}
